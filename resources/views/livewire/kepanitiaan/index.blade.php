        ...
        <table class="min-w-full text-sm text-gray-700">
            <thead class="bg-blue-100 text-gray-800 text-left uppercase tracking-wider text-xs">
                <tr>
                    ...
                    <th class="px-6 py-4 font-semibold">Organisasi</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($data as $row)
                    <tr class="hover:bg-blue-50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->kegiatan->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->anggota->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->anggota->organisasi->nama_organisasi }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->jabatan }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-3">
                                <a href="{{ route('kepanitiaan.edit', $row->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500 italic">Belum ada data kepanitiaan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
